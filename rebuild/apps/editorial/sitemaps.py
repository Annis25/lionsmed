...
from .models import NewsArticle,EditorialSection

class PublicSitemap(Sitemap):
    def items(self):
        if not settings.PUBLIC_INDEXING_ENABLED:return []
        paths=["core:home","editorial:club","actions:list","editorial:news","agenda:list","editorial:join","communications:contact","editorial:sitemap"]
        ...
        return paths+list(Action.objects.public())+list(NewsArticle.objects.public())+list(Event.objects.public())
