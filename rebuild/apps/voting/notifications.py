...
def notify_vote_opened(vote):
    for elector in Elector.objects.filter(vote=vote).select_related("profile__user"):
        notify(recipient=elector.profile.user, category="VOTE", title=f"Vote ouvert — {vote.title}",
            excerpt=("Un scrutin nominatif est ouvert : votre choix sera visible par l’administrateur du site après la clôture."
                     if vote.disclosure == "NOMINATIVE" else "Un scrutin est ouvert. Consultez les modalités dans votre espace."),
